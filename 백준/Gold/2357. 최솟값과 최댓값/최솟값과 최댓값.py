...
Mintree = [0] * (4 * len(arr))
INT = 10 ** 15
def buildMaxTree(start, end, node):
    if start == end:
        Maxtree[node] = arr[start]
        return Maxtree[node]
    mid = (start + end) // 2
    Maxtree[node] = max(buildMaxTree(start, mid, node * 2), buildMaxTree(mid + 1, end, node * 2 + 1)) 
    return Maxtree[node]
...
buildMaxTree(0, len(arr) - 1, 1)

for _ in range(m):
    a, b = map(int, input().split())
    maxNum = queryMax(0, len(arr) - 1, 1, a - 1, b - 1)
